...
type SpecReviewStatus = 'pending' | 'approved' | 'rejected';
...
function getFileExtension(fileName: string): SpecFileExtension | null {
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use((req, res, next) => {
        ...
        void (async () => {
          try {
            const { writeSpecIndexFile } = await import(
              pathToFileURL(path.join(rootDir, 'scripts', 'generate-spec-index.mjs')).href
            );
            ...
            if (req.method === 'PATCH' && pathname === '/status') {
              ...
              let extension: SpecFileExtension | null = null;
              for (const ext of ['json', 'yaml', 'yml'] as const) {
                const candidate = path.join(dir, `openapi.${ext}`);
                if (fs.existsSync(candidate)) {
                  filePath = candidate;
