...
/**
 * Re-drive DLQ messages back to the **ingest** queue.
 */
export async function main(
  ...
  for (const record of event.Records) {
    ...
    try {
      ...
      logger.info({ event: 'ingest_dlq_replayed', recordId });
    } catch (error) {
      logger.error({
        event: 'ingest_dlq_replay_failed',
        recordId,
