...
test -f src/utils/devices.json || find "$CODEBUILD_SRC_DIR" -path '*/src/utils/devices.json' -type f | head -1 | grep -q .
...
# Serverless embeds S3Bucket/S3Key in the CF template; copy it as packaged.yaml for deploy.
