...
STAGE="${STAGE:-dev}"
DEPLOYMENT_BUCKET="${DEPLOYMENT_BUCKET:-${STAGE}-mvx-device-service-bucket}"
...
EXPECTED_BUCKET="${STAGE}-mvx-device-service-bucket"
...
case "$PIPELINE_HINT" in
  *stg*|*STG*)
    if [ "$STAGE" != "stg" ]; then
      ...
      echo "  apps/device-service/stg-buildspec.yml"
      echo "Expected: STAGE=stg DEPLOYMENT_BUCKET=stg-mvx-device-service-bucket"
      exit 1
    fi
    ;;
  *prd*|*PRD*|*prod*|*PROD*)
    if [ "$STAGE" != "prd" ]; then
      ...
      echo "Fix buildspec path: apps/device-service/prd-buildspec.yml"
      exit 1
    fi
    ;;
  *dev*|*DEV*)
    if [ "$STAGE" != "dev" ]; then
      ...
      echo "Fix buildspec path: apps/device-service/buildspec.yml"
      exit 1
    fi
    ;;
esac

SERVICE_DIR="${CODEBUILD_SRC_DIR:-}/apps/device-service"
...
test -f src/utils/devices.json || find "${CODEBUILD_SRC_DIR:-$SERVICE_DIR}" -path '*/src/utils/devices.json' -type f | head -1 | grep -q .
...
EXPECTED_TABLE="device-table-${STAGE}"
EXPECTED_BUS="device-service-bus-${STAGE}"
...
for wrong in dev stg prd; do
  if [ "$wrong" != "$STAGE" ] && grep -q "device-table-${wrong}" packaged.yaml; then
    echo "ERROR: packaged.yaml contains resources for wrong stage: device-table-${wrong}"
    exit 1
  fi
done
echo "Stage check OK"
