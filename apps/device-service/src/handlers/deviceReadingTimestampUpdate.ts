...
const deviceReadingTimestampUpdateImpl: any = async (event: any, context?: Context) => {
  ...
  if (!userId) {
    ...
    return ApiResponse.badRequest('COMMON.BAD_REQUEST', {  correlationId: correlationId, event }, { code: 'BAD_REQUEST', details: [{ message: 'userId is required in path' }] });
  }

  if (!configDeviceId) {
    const duration = Date.now() - startTime;
    logHttpRequest(logger, event.httpMethod || 'PUT', event.path || '/devices/{userId}/{configDeviceId}/update', 400, duration, correlationId);
    return ApiResponse.badRequest('COMMON.BAD_REQUEST', {  correlationId: correlationId, event }, { code: 'BAD_REQUEST', details: [{ message: 'configDeviceId is required in path' }] });
  }
  ...
  if (lastReadingTimeStamp === undefined || lastReadingTimeStamp === null) {
    const duration = Date.now() - startTime;
    logHttpRequest(logger, event.httpMethod || 'PUT', event.path || `/devices/${userId}/${configDeviceId}/update`, 400, duration, correlationId);
    return ApiResponse.badRequest('COMMON.BAD_REQUEST', {  correlationId: correlationId, event }, { code: 'BAD_REQUEST', details: [{ message: 'lastReadingTimeStamp is required in request body' }] });
  }

  if (typeof lastReadingTimeStamp !== 'number' || lastReadingTimeStamp < 0) {
    const duration = Date.now() - startTime;
    logHttpRequest(logger, event.httpMethod || 'PUT', event.path || `/devices/${userId}/${configDeviceId}/update`, 400, duration, correlationId);
    return ApiResponse.badRequest('COMMON.BAD_REQUEST', {  correlationId: correlationId, event }, { code: 'BAD_REQUEST', details: [{ message: 'lastReadingTimeStamp must be a valid positive number' }] });
  }
  ...
  try {
    ...
    return ApiResponse.internalServerError('DEVICE.READING_TIMESTAMP_UPDATE_FAILED', {  correlationId: correlationId, event }, { code: 'UPDATE_FAILED' });
  }
};
