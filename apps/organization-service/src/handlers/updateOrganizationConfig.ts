...
/**
 * `PUT /organization/{organizationId}/config` — saves org config as a new draft version
 * (`CONFIG#v{n}`, `status = draft`). No Metadata Registry validation, no publish, no event.
 */
const handler = async (req: LambdaRequest<Params>) => {
  ...
  return organizationService.saveOrganizationConfig(organizationId, validationResult.data, correlationId, createdBy);
};
