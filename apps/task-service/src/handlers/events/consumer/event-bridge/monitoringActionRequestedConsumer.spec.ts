...
import { handler, main, processMonitoringActionRequested } from './monitoringActionRequestedConsumer';
...
describe('monitoringActionRequestedConsumer', () => {
  ...
  it('creates monitoring task via TaskService', async () => {
    ...
    await processMonitoringActionRequested(monitoringActionRequestedSample);
  ...
  it('returns skippedDuplicate outcome from service without throwing', async () => {
    ...
    await expect(processMonitoringActionRequested(monitoringActionRequestedSample)).resolves.toBeUndefined();
    expect(mockCreateMonitoringAction).toHaveBeenCalledTimes(1);
  });
});
