...
// eslint-disable-next-line no-var
var mockCompleteLinkedSourceObject: jest.Mock;
...
describe('task event consumers', () => {
  ...
  it('processServiceFlowActivated calls createRuntimeTask', async () => {
    mockCreateRuntimeTask.mockResolvedValue({ record: { runtimeTaskInstanceId: 'rtask-1' } });
    await processServiceFlowActivated(serviceFlowSample);
  ...
  it('processCarePlanTaskGenerationTriggered calls generateCarePlanTasks', async () => {
    mockGenerateCarePlanTasks.mockResolvedValue({ results: [] });
    await processCarePlanTaskGenerationTriggered(carePlanSample);
    expect(mockGenerateCarePlanTasks).toHaveBeenCalledWith(
      expect.objectContaining({
        organizationId: 'org-1',
        linkages: expect.any(Array),
      }),
    );
  });

  it('processLinkedSourceObjectCompleted calls completeLinkedSourceObject', async () => {
