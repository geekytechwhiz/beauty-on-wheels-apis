...
import { runRegisterReminder } from './register-reminder.processor';
...
describe('runRegisterReminder', () => {
  ...
  it('registers reminder schedule and persists LOOKUP history', async () => {
    await runRegisterReminder(basePayload, 'corr-1');
  ...
  it('skips registration without throwing when mapper returns null (no channel)', async () => {
    ...
    await runRegisterReminder(payload, 'corr-1');
