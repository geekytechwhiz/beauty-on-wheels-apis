...
| 8  | **Idempotency ⭐**                            | **Async:** `IdempotencyStrategy` in event-platform orchestrator. **HTTP:** domain conditional writes in repositories (no middleware idempotency table) | `event-platform` + domain repos | Implemented (see [IDEMPOTENCY_EVENT_PLATFORM_VS_MIDDLEWARE.md](../engineering/IDEMPOTENCY_EVENT_PLATFORM_VS_MIDDLEWARE.md)) | Async: skip handler on duplicate (store mode) or repo dedupe; HTTP: conditional put on business key |
