 ...
 * transaction race handling; persists only through {@link AlertRepository}. No HTTP envelope or JWT parsing.
 *
...
export class AlertService {
   ...
   *
   * Flow: resolve EVENT# item → reject cross-tenant idempotency key → optional patient/org HTTP validation →
   * Dynamo `TransactWriteItems` (repository). On transaction race, re-resolve idempotency and treat as duplicate if found.
   *
