...
1. **`consumeEvent(deps, registry, beforeDispatch?)`** — low-level; batch SQS via `extractRecords`, per-record `processSingle`; optional `deps.batchConcurrency` (`src/engine/executor/consume-event.ts`).
2. **`createEventHandler`** — preferred when using the middleware stack; default deps include domain idempotency, retry, DLQ enabled.
3. **`EventConsumer`** — parse → prepare → orchestrate in one call; structured `HandleResult` or throws `RETRY_EVENT` for transport retry (`src/sdk/consumer/event-consumer.ts`).
4. **`createStreamHandler`** — DynamoDB Streams; map record → `BaseEvent | null`, partial batch failures (`src/lib/create-stream-handler.ts`).
