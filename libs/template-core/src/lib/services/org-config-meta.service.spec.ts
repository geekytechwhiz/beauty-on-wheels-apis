...
describe('OrgConfigMetaService', () => {
  it('upserts with active and version defaults', async () => {
    ...
    const result = await svc.upsertOrgConfigMeta('ENABLE_SCOPE', payload);
  ...
  it('increments version when omitted on update', async () => {
    ...
    const updated = await svc.upsertOrgConfigMeta('ORG_DRAWER', { examples: { a: 1 } });
