...
export class OrangeWebhookAdapter implements LabWebhookAdapter {
  ...
  authenticate(req: APIGatewayProxyEvent): void {
    ...
    // TODO: validate timestamp and signature using partner config secret
  ...
  parseEvent(payload: WebhookPayload): CanonicalLabEvent {
    const eventType = mapOrangeEventType(payload['type'] ?? payload['eventType'] ?? payload['event_type']);
    const labOrderId = String(payload['orderId'] ?? payload['order_id'] ?? payload['lab_order_id'] ?? '');
    const occurredAt =
...
function mapOrangeEventType(value: unknown): CanonicalLabEvent['eventType'] {
  const s = typeof value === 'string' ? value.toUpperCase().replace(/-/g, '_') : '';
...
export function createOrangeAdapter(partnerId: string): LabWebhookAdapter {
  return new OrangeWebhookAdapter(partnerId);
}
