"use strict";
/**
 * HMS Client Registry Service
 * Manages HMS client credentials and launch configuration
 * Uses in-memory storage for local dev (serverless offline) when AWS credentials unavailable
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.HMSClientService = void 0;
const client_dynamodb_1 = require("@aws-sdk/client-dynamodb");
const lib_dynamodb_1 = require("@aws-sdk/lib-dynamodb");
const logger_1 = require("../utils/logger");
const TABLE_NAME = process.env.HMS_CLIENTS_TABLE || '';
const USE_IN_MEMORY = process.env.IS_OFFLINE === 'true' || process.env.USE_IN_MEMORY_STORAGE === 'true';
// In-memory store for local development (no AWS credentials needed)
const memoryStore = new Map();
let docClient = null;
if (!USE_IN_MEMORY && TABLE_NAME) {
    try {
        const client = new client_dynamodb_1.DynamoDBClient({});
        docClient = lib_dynamodb_1.DynamoDBDocumentClient.from(client);
    }
    catch {
        logger_1.logger.warn('DynamoDB client init failed, using in-memory storage');
        docClient = null;
    }
}
class HMSClientService {
    useDynamo() {
        return !USE_IN_MEMORY && !!docClient && !!TABLE_NAME;
    }
    async getClient(clientId) {
        if (this.useDynamo()) {
            try {
                const result = await docClient.send(new lib_dynamodb_1.GetCommand({
                    TableName: TABLE_NAME,
                    Key: { clientId },
                }));
                return result.Item || null;
            }
            catch (error) {
                logger_1.logger.warn('DynamoDB get failed, falling back to in-memory', { error: error.message });
                return memoryStore.get(clientId) || null;
            }
        }
        return memoryStore.get(clientId) || null;
    }
    async getClientByHmsId(hmsId) {
        if (this.useDynamo()) {
            try {
                const result = await docClient.send(new lib_dynamodb_1.GetCommand({
                    TableName: TABLE_NAME,
                    Key: { clientId: hmsId },
                }));
                return result.Item || null;
            }
            catch {
                for (const client of memoryStore.values()) {
                    if (client.hmsId === hmsId)
                        return client;
                }
                return null;
            }
        }
        for (const client of memoryStore.values()) {
            if (client.hmsId === hmsId)
                return client;
        }
        return null;
    }
    async validateClient(clientId, clientSecret) {
        const hmsClient = await this.getClient(clientId);
        if (!hmsClient)
            return null;
        if (!hmsClient.active)
            return null;
        if (clientSecret && hmsClient.clientSecret !== clientSecret)
            return null;
        return hmsClient;
    }
    async registerClient(hmsClient) {
        const fullClient = {
            ...hmsClient,
            createdAt: new Date().toISOString(),
            active: true,
        };
        if (this.useDynamo()) {
            try {
                await docClient.send(new lib_dynamodb_1.PutCommand({
                    TableName: TABLE_NAME,
                    Item: fullClient,
                }));
            }
            catch (error) {
                logger_1.logger.warn('DynamoDB put failed, using in-memory', { error: error.message });
                memoryStore.set(fullClient.clientId, fullClient);
            }
        }
        else {
            memoryStore.set(fullClient.clientId, fullClient);
        }
        logger_1.logger.info('HMS client registered', { hmsId: hmsClient.hmsId, clientId: hmsClient.clientId });
        return fullClient;
    }
}
exports.HMSClientService = HMSClientService;
//# sourceMappingURL=hms-client.service.js.map